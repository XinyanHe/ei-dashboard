...
tmux split-window -- /bin/sh -c "LD_PRELOAD=$(clang -print-file-name=libclang_rt.asan-x86_64.so) ./pox.py config=ext/ei/ei.cfg web ei-dashboard ei.snm --dom_id=$DOM --sn_id=$SN ei.snm:connection=ptcp:127.0.$DOM.$SN:$DOM${SN}099"
